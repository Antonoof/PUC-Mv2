...
# Инициализация каскадов Haar для детекции лица
try:
...
def update_bomb_game():
    ...
    if game_state.bomb_active and not game_state.bomb_exploded and game_state.bomb_start_time:
        ...
        base_time = max(2.0, 8.0 - game_state.bomb_level * 0.5)  # Минимум 2 секунды
...
def process_frame(frame):
    ...
    if face_rect is not None:
        ...
        # Рисуем прямоугольник лица ТОЛЬКО в debug режиме
        ...
        if game_state.active_game == "pushup":
            ...
            # Логика подсчета отжиманий - ИСПРАВЛЕНО!
            if face_center_y < up_line:
                state_text = "UP"
                state_color = (0, 255, 0)
                if game_state.pushup_state != "up":
                    game_state.pushup_state = "up"
                    
            elif face_center_y > down_line:
                state_text = "DOWN"
                state_color = (0, 0, 255)
                if game_state.pushup_state != "down":
                    game_state.pushup_state = "down"
                    # Если было движение из up в down, считаем отжимание
            ...
            if face_center_y < up_line:
                state_text = "STAND"
                state_color = (0, 255, 0)
                if game_state.squat_state != "stand":
                    game_state.squat_state = "stand"
                    
            elif face_center_y > down_line:
                state_text = "SQUAT"
                state_color = (255, 0, 0)
                if game_state.squat_state != "squat":
                    game_state.squat_state = "squat"
                    # Если было движение из stand в squat, считаем приседание
